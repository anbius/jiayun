...
const HTTP = 'http://jk.jianong.com';
 ...
 * @return string
 */
function jpush_user($content = "",$type='',$receive="",$txt ="",$uid = 0){
	//推送成功，添加到消息列表
	/*$add_time = date("Y-m-d H:i:s",time());
	if($uid)D("msg")->add(array('uid'=>$uid,'content'=>$content,'add_time'=>$add_time));*/
	
    $app_key = '6f5e91fbc5722e971cb0d75a';
    $master_secret = '059dfdb2eb5aff849e85fd6e';
 ...
 * @return string
 */
function jpush_doc($content = "",$type='',$receive="",$txt ="" , $did = 0){	
	//推送成功，添加到消息列表
	/*$add_time = date("Y-m-d H:i:s",time());
	if(is_array($did)){		
		foreach($did as $v){
			D("msg")->add(array('did'=>$v,'content'=>$content,'add_time'=>$add_time)); 		
		}
	}elseif(!is_array($did) && $did){
		D("msg")->add(array('did'=>$did,'content'=>$content,'add_time'=>$add_time)); 		
	} */
	
	
	
    $app_key = '7be21805b2d1b1bb41650b02';
    $master_secret = '7b2de5a99c0fb1565a2753e2';
    $pushObj = new \Org\Jpush($app_key,$master_secret);
