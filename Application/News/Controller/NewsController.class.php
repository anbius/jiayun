...
class NewsController extends AdminController{
    ...
    function _initialize()
    {
		...
		$this->bannerModel = D('News/Banner');
    }
	...
	public function banner($page = 1, $r = 10) {
		
		$builder = new AdminListBuilder();
//		if ($_GET['work_name']){
//            $map['work_name'] = array('like','%'.$_GET['work_name'].'%');
//        }
//        if ($_GET['user_name']){
//            $map['user_name'] = array('like','%'.$_GET['user_name'].'%');
//        }
//		if ($_GET['mobile_phone']){
//            $map['mobile_phone'] = $_GET['mobile_phone'];
//        }
		$map['status'] = 1;
		$data = $this->bannerModel -> where($map) ->select();
		$totalCount = count($data);
		$list = $this->bannerModel -> where($map) -> order('id desc') -> limit(($page - 1) * $r, $r) ->select();
		$builder -> title('Banner广告管理') 
				 -> data($list) 
				 -> setSelectPostUrl(U('Admin/News/banner')) 
				 -> setStatusUrl(U('News/setBannerStatus')) 
				 -> buttonNew(U('News/editBanner'))
				 -> buttonDelete() -> keyId() 
				 -> keyText('title', '标题') 
				 -> keyText('content', '简介') 
				 -> keyImage('cover', '图片') 
				 ...
				 -> keyDoActionEdit('editBanner?id=###') 
				 -> pagination($totalCount, $r)
				 -> display();
	}
	public function setBannerStatus($ids, $status) {
		$builder = new AdminListBuilder();
		$builder -> doSetStatus('banner', $ids, $status);
	}
	public function editBanner()
    {
        ...
        if(IS_POST){
            ...
            $data['sort']=I('post.sort','','op_t');
            $data['create_time']=I('post.create_time','','op_t');
			$time = time();
			$data['update_time']  = date('Y-m-d',$time);
            $result=$this->bannerModel->editData($data);
            if($result){
                $aId=$aId?$aId:$result;
                $this->success($title.'成功！',U('News/banner'));
            }else{
                $this->error($title.'失败！',$this->bannerModel->getError());
            }
        }else{
            if($aId){
                $data = $this -> bannerModel -> find($aId);
			}
            $builder=new AdminConfigBuilder();
            $builder->title($title.'价格范围')
                ->data($data)
                ->keyId()
                ->keyText('title','标题')
                ->keyText('content','简介')
                ->keySingleImage('cover','上传图片')
                ->keyText('url','广告链接URL')
                ->keyText('sort','排序')
...
//分类管理end
    ...
    public function index($page=1,$r=20)
    {
        ...
        $builder->title(L('_NEWS_LIST_'))
            ...
            ->keyId()->keyText('title',L('_TITLE_'))->keyText('category',L('_CATEGORY_'))->keyText('description',L('_NOTE_'))->keyText('sort',L('_SORT_'))
    ...
    public function editNews()
    {
        ...
        if(IS_POST){
            ...
            $builder->title($title.L('_NEWS_'))
                ...
                ->keyText('description',L('_NOTE_'))
                ->keyMultiImage('cover',L('_COVER_'))
                ...
                ->group(L('_BASIS_'),'id,uid,title,source,author,cover,content,category')
                ->group(L('_EXTEND_'),'description,view,comment,sort,dead_line,position,status')
